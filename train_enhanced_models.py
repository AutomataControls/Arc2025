#!/usr/bin/env python3
"""
Enhanced Model Training Script for ARC Prize 2025
Uses reconstruction loss to predict actual output grids instead of pattern classification
"""
...
PATTERN_WEIGHT = 0.1  # Small weight for pattern prediction auxiliary loss
...
class ARCDatasetEnhanced(Dataset):
    ...
    def __init__(self, data_dir: Path, split: str = 'train'):
        ...
        self.split = split
        self.samples = []
        self.pattern_labels = []  # Keep for auxiliary loss
    ...
    def _load_data(self):
        ...
        for task_id, task_data in challenges.items():
            ...
            # Add training examples (with known outputs)
    ...
    def _infer_pattern_type(self, input_grid: np.ndarray, output_grid: np.ndarray) -> int:
        """Infer pattern type from input-output pair (simplified)"""
    ...
    def _augment_data(self):
        ...
        logger.info(f"Added {len(augmented)} augmented samples")
...
class ReconstructionLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            pred: Predicted grid logits (B, C, H, W)
            target: Target grid one-hot (B, C, H, W)
            mask: Optional mask for valid grid cells
        """
        B, C, H, W = pred.shape
...
def train_enhanced_models():
    ...
    pattern_loss_fn = nn.CrossEntropyLoss()  # For auxiliary pattern prediction
    ...
    for model_name, model in models.items():
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for batch in pbar:
                ...
                # Optional: Add auxiliary losses for some models
                if 'transform_params' in outputs and hasattr(model, 'pattern_head'):
                    # Add pattern classification auxiliary loss
                    pattern_pred = model.pattern_head(outputs['transform_params'])
                    pattern_loss = pattern_loss_fn(pattern_pred, pattern_labels)
                    loss += PATTERN_WEIGHT * pattern_loss
            ...
            if avg_val_loss < best_val_loss:
                ...
                }, MODEL_DIR / f'{model_name}_enhanced_best.pt')
        ...
        }, MODEL_DIR / f'{model_name}_enhanced_final.pt')
    ...
    with open(MODEL_DIR / 'enhanced_training_summary.pkl', 'wb') as f:
    ...
    for model_name, history in training_history.items():
        ...
        logger.info(f"{model_name.upper()}: Final Acc: {final_acc:.2f}%, Best Acc: {best_acc:.2f}%")
...
def plot_training_results(history: Dict):
    ...
    plt.tight_layout()
    plt.savefig(MODEL_DIR / 'enhanced_training_results.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    print("Next step: Run evaluation to test ensemble performance.")